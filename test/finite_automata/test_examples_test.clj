(ns finite-automata.test-examples-test
  (:require [clojure.test :refer :all]
            [finite-automata.test-examples :refer :all]))

(def all-pass-config [{:name "odd number of zeroes"
                       :type "dfa"
                       :tuple {:states #{:q1 :q2}
                               :alphabets #{"0" "1"}
                      ...
                      {:name "alternate characters beginning and ending with same letter"
                       :type "nfa"
                       :tuple {:states #{:q1 :q2 :q3 :q4 :q5 :q6 :q7}
                               :alphabets #{"0" "1"}
                               :delta {:q1 {:e #{:q2 :q5}}
                                       :q2 {0 #{:q3}}
                                       :q3 {1 #{:q4}}
...
(deftest run-fa-tests-tests
  (testing "all tests are run"
    (is (= 11 (count (run-fa-tests all-pass-config))))))

(deftest have-all-passed-tests
  (testing "when tests have passed"
    (let [test-results (run-fa-tests all-pass-config)]
      (is (= 0 (count (all-failed test-results)))))))
